...
def make_monitors(object_names: Sequence[str], mode: int, terminal: int) -> List[Monitor]:
    ...
    for object_name in track(object_names, "Making monitors..."):
        ...
        # The `element` argument should be in the format classname.elementname (which is why we pass
        # `object_name` to it).
...
def make_monitor_data(
    channel: int, dss: win32com.client.CDispatch, monitors: Sequence[Monitor]
) -> pd.DataFrame:
    """
    ...
    monitors : list of monitor_factory.Monitor
        The monitors to read data from.
